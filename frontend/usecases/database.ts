...
export async function findSimilarRoutes(repo: IDatabaseRepository, dbFile: File, modelId?: string): Promise<SimilarResult> {
  const online = await checkBackendHealth(repo);
  if (!online) throw new Error('Backend недоступен');
  return repo.uploadSimilar(dbFile, modelId);
}
