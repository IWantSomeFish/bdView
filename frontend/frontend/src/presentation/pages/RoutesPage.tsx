...
const RoutesPage: React.FC = () => {
  ...
  const similarResult: SimilarResult | null = location.state?.similarResult
    ?? (() => { try { const s = sessionStorage.getItem('routes_similar'); return s ? JSON.parse(s) : null; } catch { return null; } })();
  const result: ParseResult | null = location.state?.result
    ?? (() => { try { const s = sessionStorage.getItem('routes_result'); return s ? JSON.parse(s) : null; } catch { return null; } })();

  console.log('RoutesPage state:', { result: result?.length, similarResult: similarResult?.length });
