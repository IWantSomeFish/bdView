...
export function useDatabase() {
  ...
  const similar = async (file: File) => {
    ...
    try {
      ...
      try {
        sessionStorage.setItem('routes_result', JSON.stringify(parseData));
        sessionStorage.setItem('routes_similar', JSON.stringify(similarData));
      } catch { /* игнорируем если переполнен */ }
    } catch (err: unknown) {
